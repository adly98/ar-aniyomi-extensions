package aniyomi.lib.mixdropextractor

import aniyomi.lib.jsunpacker.JsUnpacker
import eu.kanade.tachiyomi.animesource.model.Track
import eu.kanade.tachiyomi.animesource.model.Video
import eu.kanade.tachiyomi.network.GET
import eu.kanade.tachiyomi.util.asJsoup
import okhttp3.OkHttpClient
import java.net.URLDecoder

class MixDropExtractor(private val client: OkHttpClient) {

    fun isSupported(url: String) = MIX_DROP_REGEX.containsMatchIn(url)

    fun videosFromUrl(
        url: String,
        quality: String = "",
        externalSubs: List<Track> = emptyList(),
    ): List<Video> {
        val doc = client.newCall(GET(url)).execute().asJsoup()
        val unpacked = doc.selectFirst("script:containsData(eval):containsData(MDCore)")
            ?.data()
            ?.let(JsUnpacker::unpackAndCombine)
            ?: return emptyList()

        val videoUrl = "https:" + VIDEO_URL_REGEX.find(unpacked)?.value

        val subs = unpacked.substringAfter("Core.remotesub=\"").substringBefore('"')
            .takeIf(String::isNotBlank)
            ?.let { listOf(Track(URLDecoder.decode(it, "utf-8"), "sub")) }
            ?: emptyList()

        val quality = "MixDrop: ${quality.ifBlank { "Mirror" }}"

        return Video(videoUrl, quality, videoUrl, subtitleTracks = subs + externalSubs).let(::listOf)
    }

    companion object {
        private val MIX_DROP_REGEX by lazy { Regex("""(?://|\.)((?:mi*1*xdro*p\d*(?:jmk)?|md(?:3b0j6hj|bekjwqa|fx9dc8n|y48tn97|zsmutpcvykb))\.(?:c[ho]m?|top?|bz|gl|club|click|vc|ag|pw|net|is|s[ibx]|nu|m[sy]|ps))/(?:f|e)/(\w+)""") }
        private val VIDEO_URL_REGEX by lazy { Regex("""[^\s\"'<>\\]{7,}\.(?:mp4)(?:\?[^\"'<>\\]*)?""") }
    }
}
